...
// Relative rotation: result such that a = result * b
...
private:
    ...
    // =====================================================================
        ...
        // --- tunables ---------------------------------------------------------
        double    m_fkxyStableRange   = 0.03;
        double    m_pelvisStillRad    = 0.12;
        int       m_pelvisStillTicksThresh = 8;
        // Confidence hysteresis / LP rates
        double    m_confCommit        = 0.70;
        double    m_confRelease       = 0.30;
        double    m_confRiseRate      = 0.30;
        double    m_confFallRate      = 0.10;
        // Offset blend rates
        double    m_offsetRatePrimary = 0.18;   // one anchor dominates
        double    m_offsetRateDouble  = 0.10;   // both active
        double    m_zRatePelvisMoving = 0.40;   // pose transitions
        double    m_zRatePelvisStill  = 0.04;   // pose held
        // Pose-aware Z drift kill
        double    m_zDriveRate        = 0.02;
        int       m_poseStableTicks   = 45;     // 0.5 s @ 90 Hz
        // ZUPT
        int       m_zuptTicksThresh   = 45;
        // Pose thresholds
...
private:
    ...
    //   → Done (goNext())
    enum class CalibPhase { Idle, PrepT, CaptureT, PrepN, CaptureN, Settle, Done };
    ...
    // Legacy accumulators kept as pointers that the capture tick rebinds
    // based on the current phase — avoids two copies of the loop body.
...
public:
    ...
    // Push one pose frame.  Safe to call from the render tick.
...
private:
    ...
    std::array<Quat, kXsensSegmentCount> m_calibOffset{};
