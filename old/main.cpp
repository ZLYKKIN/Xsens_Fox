...
Quat quat_diff(const Quat& a, const Quat& b)
{
    // hipose: inv(b) * a  — the rotation that takes b into a.
    return quat_mult(b.inv(), a);
}
...
void SkeletonXsens::buildLengths(const ActorConfig& actor)
{
    ...
    const double footScale  = fl / 0.26;  // foot split scales with foot length

    double armScale = 1.0;
    if (actor.armSpanCm > 0.0) {
        const double bodyWidthM = 0.259 * h;
    ...
    const std::array<float, kXsensSegmentCountWithDummies> L = {
        ...
        // ----- right arm -----
        float(0.05 * trunkScale),   // scapular stub (neck → shoulder joint)
        float(0.10 * trunkScale),   // shoulder (acromion → upper arm start)
        float(0.186 * h * armScale),// upper arm (shoulder → elbow)
        float(0.146 * h * armScale),// forearm   (elbow    → wrist)
        float(0.108 * h * armScale),// hand      (wrist    → finger tip)
        // ----- left arm (mirror) -----
    ...
    m_len = L;
    (void)footScale;              // explicit hint we use fl directly now
}
...
SkeletonXsens::addDummySegments(const std::array<Quat, kXsensSegmentCount>& s) const
{
    ...
    const double P = M_PI;
    const Quat rScap = quat_mult(s[SEG_T8],     euler_to_quat(0, -P/2, -P/2, "XYZ"));
    const Quat lScap = quat_mult(s[SEG_T8],     euler_to_quat(0, -P/2,  P/2, "XYZ"));
    const Quat rHip  = quat_mult(s[SEG_Pelvis], euler_to_quat(0,  0,   -P/2, "XYZ"));
    const Quat lHip  = quat_mult(s[SEG_Pelvis], euler_to_quat(0,  0,    P/2, "XYZ"));
...
QVector3D LocomotionSolver::update(const Quat& qR,
                                       ...
                                       double t)
    {
        ...
        const bool yawFreeze = (m_pelvisYawAngV > 0.10);
        ...
        if (!m_initialised) {
            ...
            float targetZ = 0.55f * float(m_actorHeightM);
            if (m_pose == PoseLying) targetZ = 0.0f;
            else if (m_pose == PoseSquat) targetZ = -fkMinZ;
            else if (m_pose == PoseSit)   targetZ = 0.28f * float(m_actorHeightM);
        ...
        const double pelvisRotKill = std::max(0.0, std::min(1.0, (m_pelvisAngV - 0.4) / 0.6));
        ...
        auto maybeCommitRelease = [&](double conf, bool& committed,
            ...
            if (!committed && conf >= m_confCommit) {
                QVector3D world(fk.x() + m_offsetLast.x(),
                                fk.y() + m_offsetLast.y(),
                                fk.z() + m_offsetLast.z());
                if (m_pose == PoseStand || m_pose == PoseSquat)
                    world.setZ(0.0f);
        ...
        const bool pelvisFreeze = (m_pelvisStillTicks >= m_pelvisStillTicksThresh)
                                  && m_offsetReady;
                                    ...
                                    double& outTiltCos) const
    {
        ...
        const double pelvisToFoot = -double(std::min(fkR.z(), fkL.z()));
                          ...
                          std::array<QVector3D, kFingerSegmentsHand>& outP)
{
    ...
    for (int f = 0; f < 5; ++f) {
        ...
        Quat q0 = quat_mult(axisAngleQuat(spreadAx, spread * kSpreadSign[f] * sideSign),
                            axisAngleQuat(flexAxis, a1));
        Quat q1 = axisAngleQuat(flexAxis, a2);
        Quat q2 = axisAngleQuat(flexAxis, a3);
        Quat q3 = Quat(1, 0, 0, 0);

        Quat worldQ = Quat(1, 0, 0, 0);
        QVector3D p = kFingerBaseOffset[f];
        const int baseIdx = f * 4;

        // FIX: store CUMULATIVE worldQ (in hand-local frame), not local qN.
        ...
        worldQ = quat_mult(worldQ, q3);
        p = p + vec_rotate(QVector3D(float(kFingerBoneLen[f][3]), 0, 0), worldQ);
...
static void __cdecl foxManusErgonomicsCb(const void* raw)
{
    ...
    for (std::uint32_t i = 0; i < s->dataCount && i < 32; ++i) {
        ...
        if (anyL) { parseErgoHand(&e.data[0],  true,  lQ, lP);
                    srcL = &e.data[0];  haveL = true; }
        if (anyR) { parseErgoHand(&e.data[20], false, rQ, rP);
...
void MocapReceiver::run()
{
    ...
    while (!I.stop.load()) {
        ...
        for (std::size_t t = 0; t < trackerHandles.size(); ++t) {
            ...
            // Mag magnitude normalisation — every sensor's field strength
            // varies; equalising |mag| to ~1 lets the rejection threshold
            // inside FusionAhrs work consistently.
            if (I.magNormActive && haveMag && targetSeg >= 0 &&
            ...
            if (fuseReady && targetSeg >= 0 && targetSeg < kXsensSegmentCount) {
                ...
                if (useMag) {
                    ...
                    if (mErr > 0.40f)      dynMagRej = 5.0f;
                    else if (mErr > 0.20f) dynMagRej = 20.0f;
            ...
            if (I.test && (now - I.lastDump) > 1.5) {
                ...
                for (const auto& p : pairs) {
                    ss << "  " << std::left << std::setw(10) << p.lbl
                       << std::right
                       << "  L=" << std::setw(7) << quatAngDeg(staging.quat[p.lSeg])
                       << "°  R=" << std::setw(7) << quatAngDeg(staging.quat[p.rSeg])
                       << "°  ΔLR=" << (quatAngDeg(staging.quat[p.lSeg])
                                      - quatAngDeg(staging.quat[p.rSeg]))
                       << "°\n";
                }
                ss << "  " << std::left << std::setw(10) << "spine"
                   << std::right
                   << "  pelvis=" << std::setw(7) << quatAngDeg(staging.quat[SEG_Pelvis])
                   << "°  t8=" << std::setw(7) << quatAngDeg(staging.quat[SEG_T8])
                   << "°  head=" << std::setw(7) << quatAngDeg(staging.quat[SEG_Head])
                   << "°\n";
                ss << "============================================================\n";
                std::cout << ss.str();
                std::cout.flush();
...
static const Tr kTr[] = {
    ...
    {"calib_t_prepare",    "Приготовьтесь к T-позе…",           "Prepare for T-pose…"},
    {"calib_t_capture",    "T-поза — не двигайтесь",            "T-pose — hold still"},
    {"calib_n_prepare",    "Теперь N-поза — приготовьтесь…",    "Now N-pose — prepare…"},
    {"calib_n_capture",    "N-поза — не двигайтесь",            "N-pose — hold still"},
...
void NewSessionWizard::buildPages()
{
    ...
    // ---------- Page 3 : Dimensions ----------------------------------------
    {
        ...
        lay->addSpacing(20);
        lay->addWidget(primaryBox);
        lay->addSpacing(12);
        lay->addWidget(breakdownBox);
        lay->addSpacing(12);
        lay->addWidget(m_dimsHint);
        lay->addStretch();
...
void NewSessionWizard::refreshPoseImage()
{
    ...
    QPixmap pm(nHalf ? ":/img/npose.png" : ":/img/tpose.png");
    if (!pm.isNull()) {
...
void NewSessionWizard::onCalibrationBegin()
{
    ...
    // Double-pose calibration: the render pipeline still picks the skeleton
    // default angles by poseKind; the final pose we stand in at the end of
    // the sequence is N-pose, so report that as the reference.
...
void NewSessionWizard::onCaptureTick()
{
    ...
    // ---------- CaptureT complete → arm N half ------------------
    if (m_phase == CalibPhase::CaptureT) {
        m_captureTimer.stop();
        testLog("[calib] T-pose capture complete, samples="
                + std::to_string(m_samples.size())
                + " — arming N-pose phase", m_test);

        // Rebind pointer aliases to the N-pose buffers so the next
        // CaptureN tick writes into m_*AccumN.
        ...
        // Reset both progress bars.  Without the readyBar reset the
        // operator sees it pinned at 100 % during the whole T→N prep
        // window, which looks exactly like the calibration is already
        // over and is the #1 source of "what should I do now?" confusion.
        m_countTicksLeft = kCountdownSeconds * 10;
        m_countdownBar->setValue(0);
        m_readyBar->setValue(0);
        m_goodSamples = 0;
    ...
    for (int i = 0; i < kXsensSegmentCount; ++i) {
        ...
        // Stats — straight average across both poses.
        ...
        const QVector3D meanG_all = (meanG_T * float(cT) + meanG_N * float(cN))
                                    * float(invTot);
        gyrBias[i] = QVector3D(clampBias(meanG_all.x()),
                               clampBias(meanG_all.y()),
                               clampBias(meanG_all.z()));

        // s2s: try TRIAD first, otherwise ecompass on N-pose mean.
        ...
        if (sep > 0.3f) {
            // TRIAD: R_sensor→body s.t. R·aT_s ≈ gT_b and R·aN_s ≈ gN_b.
            const Quat q_s2b = triadSolve(gT_b, gN_b, aT_s, aN_s);
            // MocapReceiver stores s2s as body→sensor; invert.
            s2s[i] = q_s2b.inv().normalized();
            s2sMode[i] = 0;
        } else {
            // Ecompass fallback (N-pose — that's the reference we settle in).
    ...
    if (m_calibStatus) m_calibStatus->setText("Settling filter (~3.5s)…");
    ...
    for (int k = 0; k < 5; ++k) {
        const int delayMs = 3500 + k * 400;
        QTimer::singleShot(delayMs, this, [this, snapshots, badCount, k]() {
    ...
    QTimer::singleShot(5400, this, [this, snapshots, badCount]() {
        if (snapshots->empty()) {
            const SuitPose post = m_rx->snapshot();
            for (int i = 0; i < kXsensSegmentCount; ++i)
        ...
        // Enforce L/R symmetry — mirror left side across the
        // sagittal (XZ) plane and average with the right; write the
        // mirrored average back as the pair.  Kills per-side mag
        // yaw drift that otherwise makes L ≠ R.
        auto mirrorY = [](const Quat& q) {
            return Quat(q.w, -q.x, q.y, -q.z);
        };
        auto avgSym = [&](int rSeg, int lSeg) {
            const Quat& qR = m_result.calibReference[rSeg];
            const Quat& qL = m_result.calibReference[lSeg];
            const Quat qLmir = mirrorY(qL);
            const double d = qR.w*qLmir.w + qR.x*qLmir.x
                           + qR.y*qLmir.y + qR.z*qLmir.z;
            const double sgn = d < 0 ? -1.0 : 1.0;
            Quat sym(0.5*(qR.w + sgn*qLmir.w),
                     0.5*(qR.x + sgn*qLmir.x),
                     0.5*(qR.y + sgn*qLmir.y),
                     0.5*(qR.z + sgn*qLmir.z));
            sym = sym.normalized();
            m_result.calibReference[rSeg] = sym;
            m_result.calibReference[lSeg] = mirrorY(sym);
        };
        avgSym(SEG_RShoulder,  SEG_LShoulder);
        avgSym(SEG_RUpperArm,  SEG_LUpperArm);
        avgSym(SEG_RForearm,   SEG_LForearm);
        avgSym(SEG_RHand,      SEG_LHand);
        avgSym(SEG_RUpperLeg,  SEG_LUpperLeg);
        avgSym(SEG_RLowerLeg,  SEG_LLowerLeg);
        avgSym(SEG_RFoot,      SEG_LFoot);
        avgSym(SEG_RToe,       SEG_LToe);
    ...
    : QWidget(parent), m_useGloves(useGloves)
{
    ...
    auto makeSpin = [this](double def, double lo, double hi) {
        auto* s = new QDoubleSpinBox(m_bodyBox);
        s->setRange(lo, hi);
        s->setDecimals(1);
        s->setSingleStep(0.5);
        s->setValue(def);
        ...
        s->setStyleSheet(
            "QDoubleSpinBox { background:#1a1a1a; color:#FFFFFF;"
            " border:1px solid #3a3a3a; border-radius:4px; padding:2px 4px; }"
            "QDoubleSpinBox:focus { border-color:#FF7A1A; }");
        return s;
    };
    m_bodyHeight = makeSpin(175.0,  80.0, 230.0);
    m_bodyFoot   = makeSpin( 26.0,  10.0,  40.0);
    m_bodyArm    = makeSpin(  0.0,   0.0, 250.0);
    m_bodyLeg    = makeSpin(  0.0,   0.0, 130.0);

    m_bodyLblH = new QLabel(m_bodyBox);
    m_bodyLblF = new QLabel(m_bodyBox);
    m_bodyLblA = new QLabel(m_bodyBox);
    m_bodyLblL = new QLabel(m_bodyBox);
    for (auto* lab : { m_bodyLblH, m_bodyLblF, m_bodyLblA, m_bodyLblL })
        lab->setStyleSheet("color:#EAEAEA; font-weight:600; font-size:9pt;");

    auto* bGrid = new QGridLayout();
    bGrid->setHorizontalSpacing(8);
    bGrid->setVerticalSpacing(4);
    bGrid->setColumnStretch(0, 0);
    bGrid->setColumnStretch(1, 0);
    bGrid->setColumnStretch(2, 0);
    bGrid->setColumnStretch(3, 0);
    bGrid->addWidget(m_bodyLblH, 0, 0);
    bGrid->addWidget(m_bodyHeight, 0, 1);
    bGrid->addWidget(m_bodyLblF, 0, 2);
    bGrid->addWidget(m_bodyFoot,   0, 3);
    bGrid->addWidget(m_bodyLblA, 1, 0);
    bGrid->addWidget(m_bodyArm,    1, 1);
    bGrid->addWidget(m_bodyLblL, 1, 2);
    bGrid->addWidget(m_bodyLeg,    1, 3);
    ...
    bLay->addSpacing(4);
    bLay->addLayout(bGrid);

    m_bodyDebounce = new QTimer(this);
    m_bodyDebounce->setSingleShot(true);
    m_bodyDebounce->setInterval(250);
    connect(m_bodyDebounce, &QTimer::timeout, this, [this]() {
        if (m_bodySuppress) return;
        ActorConfig a;
        a.heightCm     = m_bodyHeight->value();
        a.footLengthCm = m_bodyFoot->value();
        a.armSpanCm    = m_bodyArm->value();
        a.legLengthCm  = m_bodyLeg->value();
        a.useGloves    = m_useGloves;
        emit actorChanged(a);
    });
    auto kick = [this](double) {
        if (!m_bodySuppress && m_bodyDebounce) m_bodyDebounce->start();
    };
    connect(m_bodyHeight, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, kick);
    connect(m_bodyFoot,   QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, kick);
    connect(m_bodyArm,    QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, kick);
    connect(m_bodyLeg,    QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, kick);
                               ...
                               const QVector3D& root)
{
    ...
    if (m_havePrevQ) {
        for (int i = 0; i < kXsensSegmentCount; ++i) {
            ...
            const double slowThresh   = isWrist ? 0.4  : 0.8;
            const double steadyThresh = isWrist ? 8.0  : 20.0;
            const double lockTime     = isWrist ? 0.30 : 0.50;
            ...
            if (i == SEG_RHand || i == SEG_LHand) {
                ...
                if (m_locked[i]) {
                    m_anchorLocal[i] = quat_mult(orient[i], orient[iForearm].inv()).normalized();
                    m_anchorValid[i] = true;
                    m_driftLocal[i]  = nlerpQ(m_driftLocal[i], Quat(1,0,0,0),
                                              std::min(1.0, dt / 5.0));
                } else if (m_anchorValid[i]) {
                    const Quat current_local = quat_mult(orient[i], orient[iForearm].inv()).normalized();
                    ...
                    if (2.0 * std::acos(dw) > 1e-4) {
                        const Quat qFA = orient[iForearm];
                        const Quat dInv = dL.inv();
                        const Quat qCorr = quat_mult(qFA, quat_mult(dInv, qFA.inv())).normalized();
                        filtered[i] = quat_mult(qCorr, filtered[i]).normalized();
                    }
                }
            }
...
void MocapViewport::setFreezeXY(bool frozen)
{
    ...
    if (frozen) {
        ...
        if (m_skel) {
            ...
            m_freezeAnchor = QVector3D(pelvisScene.x(), pelvisScene.y(), 0);
        }
    }
    update();
}
...
struct LiveStreamSender::Impl {
    ...
    qint64       lastEmitMs = -1;             // -1 = "ещё не слали"
...
bool LiveStreamSender::start(const LiveSettings& cfg, QString* err)
{
    ...
    m_impl->timer.start();
    m_impl->lastEmitMs = -1;                  // throttle reset
    ...
    const QVector3D& pelvisPos)
{
    ...
    // Скип-кадры до выбранного fps (24/30/60) на УРОВНЕ сокета —
    // плагин получает ровно столько пакетов, сколько просили.
    if (m_cfg.fps > 0) {
    ...
    for (int i = 0; i < kXsensSegmentCount; ++i) {
        ...
        const QVector3D pHipose = (i == SEG_Pelvis) ? pelvisPos : QVector3D(0, 0, 0);
        const QVector3D p = rotateNwuToMvn(pHipose);
        appendFloatBE(body, p.x());
        appendFloatBE(body, p.y());
        appendFloatBE(body, p.z());
        const Quat q = conjugateNwuToMvn(segQuat[i]);
    ...
    const std::array<QVector3D, kFingerSegmentsHand>& leftGloveP)
{
    ...
    // Тот же throttle, что и в pushFrame — на стороне сокета.
    if (m_cfg.fps > 0) {
        const qint64 now      = m_impl->timer.elapsed();
        const qint64 periodMs = 1000 / m_cfg.fps;
        if (m_impl->lastEmitMs >= 0 && (now - m_impl->lastEmitMs) < periodMs)
    ...
    m_impl->maybeRetransmitHandshake();
    // Gloves are a body-pose extension — always sent as MXTP02 with the
    // header's fingerSegmentCount populated. Plugins that do not understand
    // the finger segments drop them after the body section.
    const quint32 ft = quint32(m_impl->timer.elapsed());
    const quint8  bodyCount = quint8(kXsensSegmentCount);
    const quint8  fingerCount = quint8(2 * kFingerSegmentsHand);
    QByteArray body;
    // Body segments (ids 1..23).
    for (int i = 0; i < kXsensSegmentCount; ++i) {
        appendInt32BE(body, i + 1);
        const QVector3D pHipose = (i == SEG_Pelvis) ? pelvisPos : QVector3D(0, 0, 0);
        const QVector3D p = rotateNwuToMvn(pHipose);
    ...
    for (int i = 0; i < kFingerSegmentsHand; ++i) {
        appendInt32BE(body, 24 + i);
        const QVector3D p = rotateNwuToMvn(leftGloveP[i]);
        appendFloatBE(body, p.x());
        appendFloatBE(body, p.y());
        appendFloatBE(body, p.z());
        const Quat q = conjugateNwuToMvn(leftGloveQ[i]);
        appendFloatBE(body, float(q.w));
        appendFloatBE(body, float(q.x));
        appendFloatBE(body, float(q.y));
        appendFloatBE(body, float(q.z));
    ...
    for (int i = 0; i < kFingerSegmentsHand; ++i) {
        appendInt32BE(body, 44 + i);
        const QVector3D p = rotateNwuToMvn(rightGloveP[i]);
        appendFloatBE(body, p.x());
        appendFloatBE(body, p.y());
        appendFloatBE(body, p.z());
        const Quat q = conjugateNwuToMvn(rightGloveQ[i]);
        appendFloatBE(body, float(q.w));
        appendFloatBE(body, float(q.x));
        appendFloatBE(body, float(q.y));
        appendFloatBE(body, float(q.z));
...
// Short SLERP helper (avoids pulling in QQuaternion dependency).
static Quat slerp(const Quat& a, const Quat& b, double t)
{
    double dot = a.w*b.w + a.x*b.x + a.y*b.y + a.z*b.z;
    Quat bb = b;
    if (dot < 0) { bb.w = -b.w; bb.x = -b.x; bb.y = -b.y; bb.z = -b.z; dot = -dot; }
    if (dot > 0.9995) {
        Quat r(a.w + t*(bb.w - a.w), a.x + t*(bb.x - a.x),
               a.y + t*(bb.y - a.y), a.z + t*(bb.z - a.z));
        return r.normalized();
    }
    const double th0 = std::acos(dot);
    const double th  = th0 * t;
    const double s0  = std::cos(th) - dot * std::sin(th) / std::sin(th0);
    const double s1  = std::sin(th) / std::sin(th0);
    return Quat(s0*a.w + s1*bb.w, s0*a.x + s1*bb.x,
    ...
    for (int s = 0; s < kXsensSegmentCount; ++s) {
        ...
        for (size_t i = 1; i + 1 < fr.size(); ++i) {
            if (d[i] > thr && d[i+1] > thr * 0.6) {
                fr[i].segQuat[s] = slerp(fr[i-1].segQuat[s],
        ...
        for (int s = 0; s < kXsensSegmentCount; ++s) {
            ...
            for (int off = 1; off <= half; ++off) {
                if (i - off >= 0) {
                    ...
                    acc = slerp(acc, fr[i - off].segQuat[s], t);
                    wAcc += w;
                }
                if (i + off < N) {
                    const double w = k[half + off];
...
static void hdRootLowpass(std::vector<RecordedFrame>& fr, int fps)
{
    ...
    auto pass = [&](std::function<QVector3D&(size_t)> get) {
        QVector3D x1{0,0,0}, x2{0,0,0}, y1{0,0,0}, y2{0,0,0};
...
static void hdRtsSmooth(std::vector<RecordedFrame>& fr)
{
    ...
    for (int i = 1; i < N; ++i) {
        for (int s = 0; s < kXsensSegmentCount; ++s)
            fwd[i][s] = slerp(fwd[i-1][s], fr[i].segQuat[s], alpha);
    }
    bwd[N-1] = fr[N-1].segQuat;
    for (int i = N-2; i >= 0; --i) {
        for (int s = 0; s < kXsensSegmentCount; ++s)
            bwd[i][s] = slerp(bwd[i+1][s], fr[i].segQuat[s], alpha);
    }
    for (int i = 0; i < N; ++i) {
        for (int s = 0; s < kXsensSegmentCount; ++s)
            fr[i].segQuat[s] = slerp(fwd[i][s], bwd[i][s], 0.5).normalized();
    }
}
            ...
            for (int j = 0; j < kFingerSegmentsHand; ++j) {
                ...
                for (int off = 1; off <= half; ++off) {
                    if (i - off >= 0) {
                        const double w = k[half - off];
                        const double t = w / (wAcc + w);
                        acc = slerp(acc, (fr[i - off].*member)[j], t);
                        wAcc += w;
                    }
                    if (i + off < N) {
                        const double w = k[half + off];
    ...
    : m_setup(wizardResult), m_test(testMode), m_rx(rx)
{
    ...
    // Pre-compute the calibration offset = inv(reference).
    for (int i = 0; i < kXsensSegmentCount; ++i)
        m_calibOffset[i] = m_setup.calibReference[i].inv();
...
void MainWindow::onRenderTick()
{
    ...
    for (int i = 0; i < kXsensSegmentCount; ++i) {
        ...
        // Сохраняем защиту от mag-heading-snap: резкий скачок при тихих
        // гироскопах — это поправка AHRS по компасу, не реальное движение.
    ...
    if (m_streamer && m_streamer->isRunning()) {
        // Pelvis position computed every tick (NOT from viewport — viewport's
        // m_lastRenderedPelvis updates only in paintGL, which freezes when
        // the Fox_Mocap window loses focus, leaving the avatar stuck).
        // We feed identity pose-quats only via cand-orientations, but the
        // pelvis Z is anchored to actor height for proper plugin scale calc.
        const float pelvisZ = float(m_setup.heightCm * 0.55 / 100.0);
        QVector3D pelvisM(0.0f, 0.0f, pelvisZ);
        if (m_skel) {
            auto kp = m_skel->computeKeypoints(qOut, QVector3D(0.0f, 0.0f, pelvisZ));
            pelvisM = kp[SEG_Pelvis] + m_viewport->lastLocoOffset();
        }
        ...
        if (gloves) {
            ...
            const Quat qRWristWorld = quat_mult(qOut[SEG_RHand],
                                                m_skel->defAngFor(SEG_RHand));
            const Quat qLWristWorld = quat_mult(qOut[SEG_LHand],
                                                m_skel->defAngFor(SEG_LHand));

            // Y-flip для левой руки (отражение Manus → Xsens body frame
            // для пальцев).
            auto yflipQuat = [](const Quat& q) {
                return Quat(q.w, -q.x, q.y, -q.z);
            };

            std::array<Quat, kFingerSegmentsHand> rGloveWorld, lGloveWorld;
            for (int i = 0; i < kFingerSegmentsHand; ++i) {
                rGloveWorld[i] = quat_mult(qRWristWorld, f.rightGloveQ[i]);
                lGloveWorld[i] = quat_mult(qLWristWorld, yflipQuat(f.leftGloveQ[i]));
            }

            m_streamer->pushFrameWithGloves(
                quint32(f.sampleCounter), qOut,
                pelvisM,
                rGloveWorld, f.rightGloveP,
                lGloveWorld, f.leftGloveP);
        } else {
            m_streamer->pushFrame(quint32(f.sampleCounter), qOut, pelvisM);
        }
    }
    ...
    if (m_test) {
        ...
        if (now - t0 > 2.0) {
            ...
            auto diffDeg = [&](const Quat& a, const Quat& b) {
                return quatAngDeg(quat_mult(a, b.inv()));
            };
...
void MainWindow::onOpenLiveWizard()
{
    ...
    if (m_skel) {
        ...
        const auto& starts = m_skel->startPts();
        for (int i = 0; i < kXsensSegmentCount; ++i) {
            cfg.tposeOriginM[i] = rotateNwuToMvn(kp[starts[i]]);
        }
    }
...
void MainWindow::finishRecording()
{
    ...
    if (m_recCfg.quality == RecordQuality::HdPostProcessing) {
        ...
        runHdPostProcessing(m_recBuffer, m_recCfg.fps, [&](int p) {
            dlg.setValue(p);
            QCoreApplication::processEvents();
        });
        dlg.close();
...
int main(int argc, char** argv)
{
    ...
    // Parse CLI early so we know whether we should attach to a console.
