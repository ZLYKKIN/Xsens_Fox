...
Predict:
    ...
    q   ← q ⊗ Exp(ω̂ · dt)
    F   = [[I - [ω̂dt]_×, -I·dt], [0, I]]
    Q   = block-diag(σ²_g · dt² · I_3, σ²_b · dt · I_3)
    P   ← F P Fᵀ + Q
"""
import numpy as np
...
class FoxKf:
    ...
    def set_prior(self, q_world_body, bias_init,
        ...
        self.q = quat_norm(np.asarray(q_world_body, dtype=float))
        self.b = np.asarray(bias_init, dtype=float).copy()
    ...
    def predict(self, gyr, dt):
        if dt <= 0:
            return
        dt = min(dt, 0.5)
        w = np.asarray(gyr, dtype=float) - self.b
    ...
    def update_acc(self, acc_unit_g):
        self.last_acc = np.asarray(acc_unit_g, dtype=float).copy()
        mag = float(np.linalg.norm(self.last_acc))
        err = abs(mag - 1.0)
    ...
    def update_zupt(self):
        ...
        K = self.P @ H.T @ Sinv
        dx = K @ innov
        self.q = quat_norm(quat_mul(exp_so3(dx[:3]), self.q))
        self.b = self.b + dx[3:]
        I6 = np.eye(6)
        self.P = (I6 - K @ H) @ self.P
        self.P = 0.5 * (self.P + self.P.T)
