...
// §XII магнитные gate (kMagnet). РАСШИРЕНЫ для коррекции ДРЕЙФА КУРСА дистальных сенсоров.
//   Корень проблемы «сломанных рук/шеи»: без опоры по магнитометру курс (yaw) предплечий/кистей/
//   головы дрейфует (gravity правит только pitch/roll). Прежний угловой gate 6° ОТВЕРГАЛ магнит
//   ровно тогда, когда курс ушёл >6° — т.е. сам отключал коррекцию дрейфа (порочный круг: магнит
//   нужен именно при рассогласовании курса). Поле ПОСЕГМЕНТНО стабильно (dipStd<1°, headStd<2°
//   на неподвижности по [fdump]) и m0 закрепляется на пост-калибровочном курсе, поэтому магнит
//   возвращает сенсор к КАЛИБРОВАННОМУ курсу. Зона влияния — только yaw (он и так был сломан).
//   norm/dip остаются стражами искажений (металл рядом меняет и норму, и наклонение).
//   Офлайн-реплей log6: открытие gate ~0%→32-79% для предплечий/ног. (formules.txt §XII)
#define KFA_MAG_NORM_REF         1.0f
#define KFA_MAG_NORM_GATE        0.15f
#define KFA_MAG_DIP_GATE_DEG     6.0f
#define KFA_MAG_ANG_GATE_DEG     30.0f
