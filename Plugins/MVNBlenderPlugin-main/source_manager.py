...
def create_target_skeleton(mocap_pose: MocapPose, previous_session_target: str) -> bpy.types.Object:
    ...
    # --- Create armature ---
    # Race-condition guard: another deferred timer may have created this
    # armature between scheduling and execution.  If it now exists, bail
    # out — the other deferred call wins.
    if armature_name in bpy.data.objects:
        return bpy.data.objects[armature_name]
    # Clear active+selection so armature_add() doesn't inherit unrelated
    # collection/transform context from whatever the user clicked.
    bpy.ops.object.select_all(action="DESELECT")
