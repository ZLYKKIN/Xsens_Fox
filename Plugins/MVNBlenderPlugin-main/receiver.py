...
_pending_target_creation: set = set()
...
def get_ready_socket(ip_address: str, port_number: int) -> socket.socket:
    ...
    "localhost" is silently rewritten to "0.0.0.0" because on some Windows
    setups it resolves to the IPv6 loopback ::1 first, and binding an
    AF_INET socket to ::1 fails (or quietly fails to receive AF_INET
    datagrams that Fox Mocap sends to 127.0.0.1). Binding to any address
    avoids the whole loopback-resolution quagmire.
    """
    if ip_address in ("localhost", "127.0.0.1", "", None):
        ip_address = "0.0.0.0"
    ...
    if message_id == "02":  # quaternion message
        ...
        try:
            ...
            if file_logger.is_open():
                prev_count = getattr(mocap_pose, "_last_seg_count", None)
                cur_count = len(quaternion_message)
                if prev_count is not None and prev_count != cur_count:
                    file_logger.log(
                        f"[gloves_state] actor={mocap_pose.actor_name} "
                        f"prev_count={prev_count} cur_count={cur_count}"
                    )
                mocap_pose._last_seg_count = cur_count
        except Exception:
            pass
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            try:
                ...
                # Don't let a single bad packet kill the receiver thread.
                file_logger.log(f"[err] process_message: {exc!r}")

        except socket.error as e:
            if e.errno == 10038:
                print("The socket has been closed.")
                file_logger.log("[err] socket closed")
                break
            else:
                message = f"Socket error: {e}"
                file_logger.log(f"[err] socket {e}")
...
def apply_quaternion_message(mocap_pose: MocapPose, quaternion_message: list) -> None:
    ...
    # has not been hand-prepared (missing source_armature, target_armature,
    # bone map, etc.). Skip the unstable path entirely when the user opted
    # into log-only.
    log_only = False
    try:
        log_only = bool(bpy.context.scene.mvn_log_only_mode)
    except Exception:
        pass
    if log_only:
        return
    ...
    try:
        ...
        if file_logger.is_open():
            file_logger.log(f"[err] process_quaternion_data: {exc!r}")
        return
    ...
    # Wrap the main-thread apply call in a swallowing guard. The legacy
    # plug-in path crashes Blender entirely if a single frame triggers an
    # exception (None source_armature, missing bone, etc.) and we lose all
    # streaming until restart. With the guard we just drop the offending
    # frame, write a breadcrumb to alllog.txt and keep streaming.
    def _safe_apply():
        try:
            apply_transforms_function(mocap_pose)
        except Exception as exc:
            try:
                file_logger.log(f"[err] apply_transforms: {exc!r}")
            except Exception:
                pass
        return None  # one-shot timer
    bpy.app.timers.register(_safe_apply)
...
def create_target_skeleton(mocap_pose: MocapPose, scale_message: list, previous_session_target_map) -> None:
    if mocap_pose.armature_object and mocap_pose.actor_name in bpy.data.objects:
        return
    if mocap_pose.actor_name in _pending_target_creation:
        return
    mocap_pose.scale_message = scale_message
    mocap_pose.process_scale_message()
    ...
    _pending_target_creation.add(mocap_pose.actor_name)
    def _deferred_create():
        try:
            source_manager.create_target_skeleton(mocap_pose, previous_session_target)
        finally:
            _pending_target_creation.discard(mocap_pose.actor_name)
    bpy.app.timers.register(_deferred_create)
    ...
    if not mocap_pose.create_object_cluster:
        ...
        def _deferred_update():
            try:
                ...
                _pending_target_creation.discard(mocap_pose.actor_name)
        bpy.app.timers.register(_deferred_update)
...
def start_receiver(ip_address: str, port_number: int, previous_session_target_map: dict) -> None:
    ...
    # Open the file-logger so receiver.py / pose.py can dump per-frame data
    # for cross-log diff against Fox Mocap's `-test` stdout. Controlled by
    # the scene-level checkbox so production users do not get a multi-MB log
    # file dropped next to every .blend without asking.
    want_test_log = False
    try:
        want_test_log = bool(bpy.context.scene.mvn_test_logging)
    except Exception:
        pass
    if want_test_log:
        try:
            ...
            bpy.ops.logging.logger(
                ...
                message_text=f"alllog.txt could not be opened: {e}")
    global sock
    stop_event.clear()
...
def stop_receiver() -> None:
    ...
    bpy.ops.logging.logger("INVOKE_DEFAULT", message_type="INFO", message_text=message)
    file_logger.log(f"[stop] t={time.time():.6f}")
