...
class ReceiverPanel(MainPanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        # --- Fox Mocap test-mode switches ---------------------------------
        add_separator(layout, scale=0.1)
        box = layout.box()
        box.label(text="Fox Mocap test mode")
        sub = box.column(align=True)
        sub.prop(context.scene, "mvn_test_logging",
                 text="Write alllog.txt next to .blend")
        sub.prop(context.scene, "mvn_auto_start_on_load",
                 text="Auto-start stream on scene load")
        sub.prop(context.scene, "mvn_log_only_mode",
                 text="Log-only mode (skip armature apply)")
